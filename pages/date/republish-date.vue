<template>
	<view class="wrapper">
		...
		<view class="publish-wrap">
			<view class="btn-publish" @click="addOrder">
				<view class="icon-wrap">
		...
		<pay-cash-modal :isShow="isPayCashShow" :columnId="columnId" :orderSn="orderSn" @close="isPayCashShow = false"></pay-cash-modal>
...
<script>
	...
	export default {
		...
		async onLoad(options) {
			...
			await this.getColumnSelect();

			this.getRepublishDate(options.orderSn);
		...
		methods: {
			...
			async getRepublishDate(id) {
				...
				this.current = this.partyTypeId == res.data.data.columnId ? 0 : 1;
				...
				if(this.current == 0) {
					...
					this.ageIndex = data.beInviteAge - 1;
					this.genderIndex = data.beInviteSex;
					this.partyTypeId = data.columnId;
					this.payIndex = data.consumerType;
					// inviteDate;
					this.partyNum = data.inviteNum;
					// this.abortIndex = data.inviteOverTime;
					this.partyLatitude = data.lat;
					this.partyLongitude = data.lon;
					this.partyTheme = data.title;
					...
					this.meetingAddress = data.address;
					this.meetingAgeIndex = data.beInviteAge -1;
					this.meetingGenderIndex = data.beInviteSex;
					this.meetingTypeIndex = this.meetingTypeArray.findIndex(item => item.value == data.columnId);
					this.meetingPayIndex = data.consumerType;
					// inviteDate;
					this.meetingNum = data.inviteNum;
					// this.meetingAbortIndex = data.inviteOverTime;
					this.meetingLatitude = data.lat;
					this.meetingLongitude = data.lon;
					this.remark = data.remark;
					this.meetingTheme = data.title;
